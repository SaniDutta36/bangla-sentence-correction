...
sess = tf.compat.v1.InteractiveSession() # initializes a tensorflow session
...
encoder_inputs_length = tf.compat.v1.placeholder(shape=(None,), dtype=tf.int32, name='encoder_inputs_length')
...
embeddings = tf.Variable(tf.random.uniform([vocab_size, input_embedding_size], -1.0, 1.0), dtype=tf.float32)

# this thing could get huge in a real world application
encoder_inputs_embedded = tf.nn.embedding_lookup(params=embeddings, ids=encoder_inputs)

from tensorflow.python.ops.rnn_cell import LSTMStateTuple
from tensorflow.keras import layers   # LSTMCell, Bidirectional

encoder_cell = tf.keras.layers.LSTMCell(encoder_hidden_units)
# Check this link out [https://github.com/tensorflow/tensorflow/issues/26974#issuecomment-475517563]
((encoder_fw_outputs,
  encoder_bw_outputs),
 (encoder_fw_final_state,
  encoder_bw_final_state)) = (tf.keras.layers.Bidirectional(layer=encoder_cell,
                                    backward_layer=encoder_cell,
                                    input_shape=encoder_inputs_embedded,
...
encoder_max_time, batch_size = tf.unstack(tf.shape(input=encoder_inputs))
...
#weights
W = tf.Variable(tf.random.uniform([decoder_hidden_units, vocab_size], -1, 1), dtype=tf.float32)
#bias
...
eos_step_embedded = tf.nn.embedding_lookup(params=embeddings, ids=eos_time_slice)
pad_step_embedded = tf.nn.embedding_lookup(params=embeddings, ids=pad_time_slice)
...
def loop_fn_transition(time, previous_output, previous_state, previous_loop_state):
    def get_next_input():
        ...
        prediction = tf.argmax(input=output_logits, axis=1)
        # embed prediction for the next input
        next_input = tf.nn.embedding_lookup(params=embeddings, ids=prediction)
        return next_input
    ...
    finished = tf.reduce_all(input_tensor=elements_finished)  # -> boolean scalar
    # Return either fn1() or fn2() based on the boolean predicate pred.
    input = tf.cond(pred=finished, true_fn=lambda: pad_step_embedded, false_fn=get_next_input)
...
# ta = tensor array
decoder_outputs_ta, decoder_final_state, _ = tf.compat.v1.nn.raw_rnn(decoder_cell, loop_fn)
...
decoder_max_steps, decoder_batch_size, decoder_dim = tf.unstack(tf.shape(input=decoder_outputs))
...
#final prediction
decoder_prediction = tf.argmax(input=decoder_logits, axis=2)

#cross entropy loss
#one hot encode the target values so we don't rank just differentiate
stepwise_cross_entropy = tf.nn.softmax_cross_entropy_with_logits(
    labels=tf.stop_gradient(tf.one_hot(decoder_targets, depth=vocab_size, dtype=tf.float32)),
    logits=decoder_logits,
)

#loss function
loss = tf.reduce_mean(input_tensor=stepwise_cross_entropy)
#train it
...
try:
    for epoch in range(1, config["n_epoch"]):
        ...
        utils.save_obj("train_stat", train_stat)
